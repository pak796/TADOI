...
describe("layoutGuard", () => {
  it("validates minimum supported terminal size", () => {
    expect(isTerminalSizeSupported(MIN_TERMINAL_WIDTH, MIN_TERMINAL_HEIGHT)).toBe(true);
    expect(isTerminalSizeSupported(MIN_TERMINAL_WIDTH - 1, MIN_TERMINAL_HEIGHT)).toBe(false);
    expect(isTerminalSizeSupported(MIN_TERMINAL_WIDTH, MIN_TERMINAL_HEIGHT - 1)).toBe(false);
  });
