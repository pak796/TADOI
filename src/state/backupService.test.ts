...
describe("buildTimestampedBackupPath", () => {
  it("resolves default backup directory from data path", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-backup-default-resolve-"),
    );
  ...
  it("ensures default backup directory exists", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-backup-default-dir-"),
    );
    const dataPath = path.join(tempDir, "tadoi_data.json");
  ...
  it("uses default backups folder and increments suffix on collisions", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-backup-path-"),
    );
    const dataPath = path.join(tempDir, "tadoi_data.json");
    ...
    expect(first).toBe(
      path.join(tempDir, "backups", "tadoi-backup-20260210-000000.json"),
    );
  ...
  it("lists recognized backup files newest-first by mtime", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-backup-list-"),
    );
  ...
  it("creates missing backup directory and returns empty list", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-backup-empty-list-"),
    );
...
describe("backupService import/export", () => {
  ...
  it("repairs local non-normalized task tags for import and reports warning", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-import-tag-repair-"),
    );
    const dataPath = path.join(tempDir, "tadoi_data.json");
    const importPath = path.join(tempDir, "incoming.json");
    await fs.writeFile(
  ...
  it("enforces JSON import size limits at boundary values", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-import-size-"),
    );
    const dataPath = path.join(tempDir, "tadoi_data.json");
    const importPath = path.join(tempDir, "incoming.json");
    await fs.writeFile(
  ...
  it("commits replace import and creates backup by default", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-import-commit-"),
    );
    const dataPath = path.join(tempDir, "tadoi_data.json");
    const importPath = path.join(tempDir, "incoming.json");
    await fs.writeFile(
    ...
    try {
      ...
      const nextJson = JSON.parse(nextRaw) as {
        tasks: Array<{ title: string }>;
      };
  ...
  it("allows commit import when current process already owns lock", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-import-reentrant-lock-"),
    );
    const dataPath = path.join(tempDir, "tadoi_data.json");
    const importPath = path.join(tempDir, "incoming.json");
    await fs.writeFile(
  ...
  it("rejects commit import when lock belongs to another pid", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-import-foreign-lock-"),
    );
    const dataPath = path.join(tempDir, "tadoi_data.json");
    const importPath = path.join(tempDir, "incoming.json");
    await fs.writeFile(
  ...
  it("imports nested notification settings and writes them to settings.json", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-import-settings-"),
    );
    const dataPath = path.join(tempDir, "tadoi_data.json");
    const importPath = path.join(tempDir, "incoming.json");
    await fs.writeFile(
    ...
    try {
      ...
      const parsedSettings = JSON.parse(
        await fs.readFile(resolvedSettingsPath, "utf8"),
      );
  ...
  it("preserves imported custom1 theme payload", async () => {
    const tempDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "tadoi-import-custom-theme-"),
    );
    const dataPath = path.join(tempDir, "tadoi_data.json");
    const importPath = path.join(tempDir, "incoming.json");
    await fs.writeFile(
    ...
    try {
      ...
      const resolvedSettingsPath = await fs
        .stat(primary)
        .then(() => primary)
        .catch(() => fallback);
      ...
      expect(parsed.customThemes?.custom1?.objects?.taskList?.panel).toBe(
        "#123456",
      );
      expect(parsed.customThemes?.textByTheme?.retro?.global?.text).toBe(
        "#A1B2C3",
      );
      expect(
        parsed.customThemes?.textByTheme?.retro?.objects?.help?.mutedText,
      ).toBe("#445566");
    } finally {
      if (originalDataPath === undefined) {
        delete process.env.TADOI_DATA_PATH;
