...
function userCompletionTargets(homeDir: string): Array<{ shell: "bash" | "zsh" | "fish"; targetPath: string }> {
  return [
    {
      shell: "bash",
    ...
    {
      shell: "fish",
      targetPath: path.join(homeDir, ".config", "fish", "completions", `${CLI_NAME}.fish`)
    }
  ];
}

function renderInvocationLine(invocation: TadoiInvocation, platform: NodeJS.Platform): string {
...
function buildMainInstallInstructions(options: {
  ...
  const currentEntry = renderInvocationLine(options.invocation, options.platform);
  const lines = [`current_cli_entry: ${currentEntry}`];

  if (options.invocation.baseArgs.length > 0) {
    lines.push("script_based_install: remove the package, wrapper, or checkout that owns the script path above");
    lines.push("common_global_examples: bun remove -g tadoi | npm uninstall -g tadoi");
    return lines;
  }
  ...
  if (options.platform === "win32") {
    lines.push("windows_installer: uninstall TADOI from Settings > Apps or Control Panel");
    lines.push("windows_manual_binary: delete the install directory that contains the current CLI entry and remove it from PATH");
    lines.push("open_new_terminal: required after removing the main install");
    return lines;
  }

  lines.push("linux_deb: remove the package with your distro package manager (example: sudo apt remove tadoi)");
  lines.push("linux_appimage_or_manual: delete the AppImage or binary that contains the current CLI entry");
  lines.push(
...
export async function runUninstallCommand(
  ...
  for (const entry of userCompletionTargets(homeDir)) {
    ...
    if (removal.status === "missing") {
      runtime.log(`${entry.shell}_completion: not present (${entry.targetPath})`);
      continue;
    }
    hadFailure = true;
    runtime.error(
