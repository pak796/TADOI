...
describe("resolveTitsCliInput", () => {
  ...
  it("resolves recur wrapper command", () => {
    expect(
      resolveTitsCliInput(["recur", "id:task-1", "every:week", "on:mon"]),
    ).toEqual({
      mode: "subcommand",
...
describe("runTitsCommandCliWithDeps", () => {
  it("returns locked exit code for write commands when lock is present", async () => {
    ...
    expect(result).toEqual({
      handled: true,
      exitCode: TITS_CLI_EXIT_CODE.LOCKED,
    });
  ...
  it("maps done/due target resolution failures to exit code 3", async () => {
    ...
    expect(errors[0]).toContain(
      "Error: done requires an existing selected task or id",
    );
  });
  ...
  it("supports done selector mode and applies bulk done to matched tasks", async () => {
    ...
    expect(saved).toHaveLength(1);
    const savedTask = saved[0]?.tasks.find(
      (task) => task.id === "task-open-work",
    );
  ...
  it("supports due selector mode with mini datetime tokens", async () => {
    ...
    expect(logs).toEqual(["Bulk due set (1 tasks)"]);
    expect(saved).toHaveLength(1);
    expect(saved[0]?.tasks[0]?.dueAt).toBe(
      new Date(2026, 1, 21, 15, 0).getTime(),
    );
    expect(saved[0]?.tasks[0]?.hasExplicitTime).toBe(true);
  });
  ...
  it("routes note delete and restore-defaults through shared notes runner", async () => {
    ...
    const deleteResult = await runTitsCommandCliWithDeps(
      ["note", "delete", "Design"],
      deps,
    );
    const restoreResult = await runTitsCommandCliWithDeps(
  ...
  it("permits only one concurrent writer when lock is already acquired", async () => {
    ...
    const secondRun = await runTitsCommandCliWithDeps(
      ["add", "Second"],
      second.deps,
    );
