...
function makeToast(
  partial: Partial<EngagementToast> & Pick<EngagementToast, "id">,
): EngagementToast {
  return {
    id: partial.id,
...
function withCompletionEvents(
  events: Array<{ at: number; tags?: string[] }>,
): EngagementState {
...
describe("engagement helpers", () => {
  ...
  it("triggers done-3-today once per day", () => {
    ...
    expect(
      evaluated.toasts.some((toast) => toast.id.startsWith("DONE_3_TODAY:")),
    ).toBe(true);
    const repeated = evaluateMilestones(evaluated.engagement, now);
    expect(
      repeated.toasts.some((toast) => toast.id.startsWith("DONE_3_TODAY:")),
    ).toBe(false);
  });
  ...
  it("triggers weekly tag milestone with cooldown", () => {
    ...
    const evaluated = evaluateMilestones(engagement, now);
    expect(
      evaluated.toasts.some((toast) => toast.id === "TAG_5_LAST_7_DAYS:work"),
    ).toBe(true);

    const cooldownCheck = evaluateMilestones(
      evaluated.engagement,
      now + 60_000,
    );
    expect(
      cooldownCheck.toasts.some(
        (toast) => toast.id === "TAG_5_LAST_7_DAYS:work",
      ),
    ).toBe(false);
  });
  ...
  it("triggers 3-day streak milestone once", () => {
    ...
    expect(evaluated.toasts.some((toast) => toast.id === "STREAK_3_DAYS")).toBe(
      true,
    );
    const repeated = evaluateMilestones(evaluated.engagement, day3);
    expect(repeated.toasts.some((toast) => toast.id === "STREAK_3_DAYS")).toBe(
      false,
    );
  });
  ...
  it("caps queued toasts and drops lowest priority first (fifo for ties)", () => {
    ...
    const next = enqueueToastsWithCap(
      queue,
      [makeToast({ id: "d", priority: 1 })],
      3,
    );
