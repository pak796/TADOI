...
export function printCalendarExportHelp(): void {
  redactedLogger.log(`Usage: ${CLI_NAME} calendar:export --out <file.ics> [options]`);
  redactedLogger.log("");
  redactedLogger.log("Options:");
  redactedLogger.log("  --out <path>                 Output .ics path (required)");
  redactedLogger.log("  --view <name>                Saved view display name");
  redactedLogger.log("  --range <next7|month|all>    Date range (default: next7)");
  redactedLogger.log("  --privacy <minimal|full>     Export metadata detail (default: minimal)");
  redactedLogger.log("  --include-details            Compatibility alias for --privacy=full");
  redactedLogger.log("  -h, --help                   Show calendar export help");
  redactedLogger.log("");
  redactedLogger.log("Notes:");
  redactedLogger.log("  - Exports open tasks only (done/archived excluded).");
  redactedLogger.log("  - Privacy default is minimal (no notes/tags/links/url in output).");
  ...
  redactedLogger.log(`  ${CLI_NAME} calendar:export --out ./tadoi.ics --range all`);
}
...
export async function runCalendarExportCommand(
  ...
  try {
    ...
    redactedLogger.log(`[calendar:export] tasks scanned: ${result.tasksScanned}`);
    redactedLogger.log(`[calendar:export] events written: ${result.eventsWritten}`);
    redactedLogger.log(
    ...
    if (result.timeContext.mode === "tzid") {
      redactedLogger.log(`[calendar:export] timezone: ${result.timeContext.timeZone}`);
    } else {
