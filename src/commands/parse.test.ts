...
describe("parseCommand", () => {
  it("parses add with unquoted multi-word title and options", () => {
    const parsed = parseCommand(
      'add Buy milk due:2026-02-28 at:17:30 #errands notes:"2%"',
    );
  ...
  it("parses note command families", () => {
    ...
    expect(
      parseCommand(
        'nq "Daily" "Body" #work --status done --alias d1 --meta:source=cli',
      ),
    ).toEqual({
      ok: true,
      command: {
    ...
    expect(
      parseCommand(
        "note search tag:inbox title:retro -tag:closed created:today limit:20",
        MINI_OPTIONS_10,
      ),
    ).toEqual({
      ok: true,
  ...
  it("parses recur clear and recur rule forms", () => {
    ...
    expect(
      parseCommand("recur @selected every:week interval:2 on:mon,wed"),
    ).toEqual({
      ok: true,
      command: {
        type: "recur",
