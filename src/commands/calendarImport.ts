...
export function printCalendarImportHelp(): void {
  redactedLogger.log(
    `Usage: ${CLI_NAME} calendar:import --in <file.ics> [options]`,
  );
  redactedLogger.log("");
  redactedLogger.log("Options:");
  redactedLogger.log(
    "  --in <path>                  Input .ics path (required)",
  );
  ...
  redactedLogger.log(
    "  --mode <merge|update|create> Import mode (default: merge)",
  );
  redactedLogger.log(
    "  --horizon-days <n>           Recurrence expansion horizon (default: 365)",
  );
  redactedLogger.log(
    "  --dry-run                    Parse and plan without writing",
  );
  redactedLogger.log(
    "  --tag <value>                Tag to add on newly created tasks only",
  );
  redactedLogger.log("  --report <path>              Write JSON import report");
  redactedLogger.log(
    "  -h, --help                   Show calendar import help",
  );
  redactedLogger.log("");
  redactedLogger.log("Notes:");
  redactedLogger.log("  - One-way import action only (no live sync).");
  redactedLogger.log(
    "  - Identity precedence: X-TADOI-TASK-ID, then TADOI UID, then external UID.",
  );
  redactedLogger.log(
    "  - merge mode is conservative (tags/links union, notes append).",
  );
  redactedLogger.log(
    "  - RRULE must be valid for recurring series; recurrence expansion is bounded.",
  );
  redactedLogger.log("");
  redactedLogger.log("Examples:");
  redactedLogger.log(
    `  ${CLI_NAME} calendar:import --in ./tadoi.ics --dry-run`,
  );
  redactedLogger.log(
...
export async function runCalendarImportCommand(
  ...
  try {
    ...
    redactedLogger.log(
      `[calendar:import] events parsed: ${result.summary.eventsParsed}${dryRunLabel}`,
    );
    redactedLogger.log(
      `[calendar:import] matched by X-TADOI-TASK-ID: ${result.summary.matchedByTaskId}`,
    );
    redactedLogger.log(
      `[calendar:import] matched by UID: ${result.summary.matchedByUid}`,
    );
    ...
    redactedLogger.log(
      `[calendar:import] overrides created: ${result.summary.overridesCreated}`,
    );
    redactedLogger.log(
      `[calendar:import] overrides updated: ${result.summary.overridesUpdated}`,
    );
    redactedLogger.log(
    ...
    if (result.outputReportPath) {
      redactedLogger.log(
        `[calendar:import] report: ${result.outputReportPath}`,
      );
    }
    ...
    return result.hasErrors
      ? CLI_EXIT_CODE.TARGET_RESOLUTION
      : CLI_EXIT_CODE.SUCCESS;
  } catch (error: unknown) {
    if (error instanceof TadoiLockBusyError) {
      redactedLogger.error(
        "[calendar:import] failed: TADOI is running (lock present).",
      );
    ...
    if (error instanceof CalendarImportFilesystemError) {
      redactedLogger.error(
        `[calendar:import] failed: ${toErrorMessage(error)}`,
      );
