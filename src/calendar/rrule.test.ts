...
describe("calendar rrule helpers", () => {
  it("gates RRULE fragments with strict validity checks", () => {
    expect(isValidRRuleFragment("FREQ=WEEKLY;INTERVAL=1;BYDAY=MO,WE")).toBe(true);
  ...
  it("formats RRULE lines with RRULE: prefix", () => {
    expect(toRRuleLine("FREQ=DAILY;INTERVAL=1")).toBe("RRULE:FREQ=DAILY;INTERVAL=1");
  ...
  it("formats EXDATE values for date and date-time forms", () => {
    ...
    expect(formatRecurrenceExdates(exdates, "date")).toEqual(["20260210", "20260211"]);
  });
});
