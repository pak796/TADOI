...
export function validateThemeTokensContrast(
  ...
  for (const rule of CONTRAST_RULES) {
    const ratio = computeContrastRatio(tokens[rule.foreground], tokens[rule.background]);
    const baselineRatio = baselineTokens
      ...
      : null;
    const baselineWasFailing = baselineRatio !== null && baselineRatio < rule.minimumRatio;
    const becameWorse =
      baselineRatio !== null && ratio < baselineRatio - 0.01;
    const shouldReport =
...
export function formatContrastIssueForBanner(issue: ThemeContrastIssue): string {
