...
describe("routingContinuations helpers", () => {
  it("maps left-rail items to task-editor continuations", () => {
    ...
    expect(resolveTaskEditorContinuationForLeftRail("BACKUP")).toBe(
      "open_backup_center",
    );
    expect(resolveTaskEditorContinuationForLeftRail("NOTES")).toBe(
      "open_notes",
    );
  ...
  it("describes continuation labels for unsaved modal copy", () => {
    ...
    expect(describeTaskEditorContinuation("open_backup_center")).toBe(
      "open Backup Center",
    );
    expect(describeTaskEditorContinuation("open_notes")).toBe("open TOME");
