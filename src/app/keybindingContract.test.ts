...
describe("link keybinding contract", () => {
  ...
  it("keeps legacy tag-cycle docs aligned with lowercase-only router behavior", async () => {
    const usagePath = fileURLToPath(
      new URL("../../docs/USAGE.md", import.meta.url).href,
    );
  ...
  it("keeps backup center key snapshot aligned with routed keyspace", async () => {
    const specPath = fileURLToPath(
      new URL("../../TADOI_SPEC_v0.4.0.md", import.meta.url).href,
    );
    ...
    expect(
      run(
        { name: "4", sequence: "4" },
        { uiState: backupModeState, backupScreen: "menu" },
      ),
    ).toEqual([{ scope: "ui", type: "BACKUP_SELECT_MENU_OPTION", index: 3 }]);
    expect(
      run(
        { name: "home", sequence: "" },
        { uiState: backupModeState, backupScreen: "import_picker" },
      ),
    ).toEqual([
    ...
    expect(
      run(
        { name: "pagedown", sequence: "" },
        { uiState: backupModeState, backupScreen: "import_dryrun" },
      ),
    ).toEqual([{ scope: "ui", type: "BACKUP_SCROLL_BODY", delta: 8 }]);
    expect(
      run(
        { ctrl: true, name: "u", sequence: "" },
        { uiState: backupModeState, backupScreen: "calendar_import_dryrun" },
      ),
    ).toEqual([{ scope: "ui", type: "BACKUP_SCROLL_BODY", delta: -8 }]);
  });
