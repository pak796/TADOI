...
describe("keymapAliases", () => {
  ...
  it("captures deterministic first-wins conflict warnings per context", () => {
    ...
    expect(resolved.byContext.list?.tokenToAction["/"]).toBe("list_open_search");
    expect(resolved.byContext.list?.tokenToAction.p).toBe("list_open_tag_panel");
  ...
  it("maps key input to alias tokens with ctrl precedence", () => {
    expect(
      keyInputToAliasTokens({ name: "s", sequence: "s", ctrl: true, shift: false })
    ).toEqual(["Ctrl+S"]);
    expect(
      keyInputToAliasTokens({ name: "up", sequence: "", ctrl: false, shift: false })
    ).toEqual(["ArrowUp"]);
    expect(
      keyInputToAliasTokens({ name: "/", sequence: "?", ctrl: false, shift: true })
    ).toEqual(["?", "/"]);
  });
