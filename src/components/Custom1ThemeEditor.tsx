...
export function resolveCustom1EditorRightFocus(
  current: FocusTarget,
): FocusTarget | null {
...
export function resolveCustom1EditorLeftFocus(
  current: FocusTarget,
): FocusTarget | null {
...
function resolveTokenJumpTarget(query: string): keyof ThemeTokens | null {
  ...
  const exact = THEME_TOKEN_KEYS.find(
    (token) => token.toLowerCase() === normalized,
  );
  if (exact) return exact;
  return (
    THEME_TOKEN_KEYS.find((token) =>
...
>(function Custom1ThemeEditor(
  ...
  const selectedToken =
    THEME_TOKEN_KEYS[selectedTokenIndex] ?? THEME_TOKEN_KEYS[0];
      ...
      handleKey(key: KeyEvent): boolean {
        ...
        if (key.name === "tab") {
          ...
          setFocusTarget((current) =>
            resolveCustom1EditorTabFocus(current, key.shift === true),
          );
          return true;
        }
        ...
        if (focusTarget === "tokenList") {
          if (key.name === "up") {
            setSelectedTokenIndex(
              (current) =>
                (current - 1 + THEME_TOKEN_KEYS.length) %
                THEME_TOKEN_KEYS.length,
            );
            return true;
          }
          if (key.name === "down") {
            setSelectedTokenIndex(
              (current) => (current + 1) % THEME_TOKEN_KEYS.length,
            );
            return true;
          }
          if (key.name === "right") {
