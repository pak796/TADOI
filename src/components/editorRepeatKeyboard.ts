...
export function cycleRepeatModeClamp(
  ...
  const nextIndex = Math.max(
    0,
    Math.min(
      REPEAT_MODE_KEYBOARD_ORDER.length - 1,
      safeCurrentIndex + direction,
    ),
  );
...
export function cycleRepeatEndModeClamp(
  ...
  const nextIndex = Math.max(
    0,
    Math.min(
      REPEAT_END_MODE_KEYBOARD_ORDER.length - 1,
      safeCurrentIndex + direction,
    ),
  );
...
export function cycleReminderKindClamp(
  ...
  const nextIndex = Math.max(
    0,
    Math.min(
      REMINDER_KIND_KEYBOARD_ORDER.length - 1,
      safeCurrentIndex + direction,
    ),
  );
...
export function shouldInterceptRepeatArrowAtEdge(params: {
  ...
  if (params.keyName === "right")
    return params.cursorOffset >= params.valueLength;
  return false;
}
