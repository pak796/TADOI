...
const WORKFLOW_STAGE_OPTIONS = [
  ...
  value: NonNullable<EditorDraft["workflowStage"]>;
  label: string;
}>;
...
function normalizeReminderOffsetUnitInput(
  input: string,
): EditorDraft["reminderOffsetUnit"] {
  return normalizeReminderOffsetUnit(input.trim().toLowerCase());
}

function normalizeWorkflowStageInput(
  input: string,
...
function cycleWorkflowStage(
  ...
  if (currentIndex === -1) {
    return direction === 1
      ...
      : (WORKFLOW_STAGE_OPTIONS[size - 1]?.value ??
          WORKFLOW_STAGE_OPTIONS[0].value);
  }
...
function parseWeekdaysInput(value: string): string[] {
  ...
  return Array.from(
    new Set(
      raw.filter((token) =>
        WEEKDAY_ORDER.includes(token as (typeof WEEKDAY_ORDER)[number]),
      ),
    ),
  );
}
...
export function runPrimaryMouseDownAction(
  button: number,
  action: () => void,
): boolean {
...
export function EditorPane({
  ...
  const { contentHeight, footerHeight } =
    getEditorViewportHeights(availableHeightLines);
  ...
  const [workflowStageInput, setWorkflowStageInput] = useState(
    draft.workflowStage ?? "",
  );
  ...
  const linksHint =
    mode === Mode.ADD
      ? ` · CTRL+L: ADD LINK/ATTACHMENT (${draft.links.length})`
      : "";
  ...
  const titleSuggestionPrefix = titleInlineSuggestion
    ? titleInlineSuggestion.full.slice(
        0,
        titleInlineSuggestion.full.length -
          titleInlineSuggestion.remainder.length,
      )
    : "";
  const beforeDueReminderMissingDue =
    reminderVisibility.kind === "before_due" &&
    draft.dueText.trim().length === 0;
  ...
  const notesScrollbar = useMemo(() => {
    ...
    const thumbSize = Math.max(
      ...
      Math.min(
        NOTES_VISIBLE_ROWS,
        Math.floor((NOTES_VISIBLE_ROWS * NOTES_VISIBLE_ROWS) / lineCount),
      ),
    );
    const travel = NOTES_VISIBLE_ROWS - thumbSize;
    const thumbTop =
      travel > 0 ? Math.round((scrollY / maxScroll) * travel) : 0;
  ...
  function handleRepeatCycleFromInputKey(
    ...
    if (source === "reminder_kind") {
      ...
      const nextReminderKind = cycleReminderKindClamp(
        draft.reminderKind,
        direction,
      );
    ...
    if (source === "repeat_end_mode") {
      ...
      key.stopPropagation();
      const direction: 1 | -1 = key.name === "right" ? 1 : -1;
      const nextRepeatEndMode = cycleRepeatEndModeClamp(
        draft.repeatEndMode,
        direction,
      );
    ...
    const inputRef =
      source === "time" ? timeInputRef.current : repeatIntervalInputRef.current;
    if (!inputRef) return;
  ...
  return (
    <box style={{ flexDirection: "column", height: "100%", minHeight: 0 }}>
      <box
        style={{ flexDirection: "column", height: contentHeight, minHeight: 0 }}
      >
        <scrollbox
          ref={scrollboxRef}
          scrollY
        ...
        >
          ...
          >
            ...
            <box style={{ flexDirection: "column", marginTop: 1 }}>
              <text style={{ color: theme.muted }}>
                TIME (HH:mm / 3pm optional)
              </text>
              <input
                ref={timeInputRef}
                value={draft.timeText}
                onChange={(value) =>
                  onUpdate({ timeText: normalizeTimeTextInput(value) })
                }
            ...
            <box style={{ flexDirection: "column", marginTop: 1 }}>
              ...
              <input
                ...
                onChange={(value) =>
                  onUpdate({ reminderKind: normalizeReminderKind(value) })
                }
                onKeyDown={(key) =>
                  handleRepeatCycleFromInputKey(key, "reminder_kind")
                }
            ...
            {reminderVisibility.showAbsolute ? (
              <>
                <box style={{ flexDirection: "column", marginTop: 1 }}>
                  <text style={{ color: fieldLabelColor(true) }}>
                    REMIND AT DATE (YYYY-MM-DD)
                  </text>
                  <input
                    value={draft.reminderAtDateText}
                    onChange={(value) =>
                      onUpdate({ reminderAtDateText: value })
                    }
                ...
                <box style={{ flexDirection: "column", marginTop: 1 }}>
                  <text style={{ color: fieldLabelColor(true) }}>
                    REMIND AT TIME (HH:mm optional)
                  </text>
                  <input
                    value={draft.reminderAtTimeText}
                    onChange={(value) =>
                      onUpdate({
                        reminderAtTimeText: normalizeTimeTextInput(value),
                      })
                    }
              ...
              <>
                <box style={{ flexDirection: "column", marginTop: 1 }}>
                  ...
                  <input
                    ...
                    onChange={(value) =>
                      onUpdate({ reminderOffsetText: value })
                    }
                ...
                <box style={{ flexDirection: "column", marginTop: 1 }}>
                  ...
                  <input
                    ...
                    onChange={(value) =>
                      onUpdate({
                        reminderOffsetUnit:
                          normalizeReminderOffsetUnitInput(value),
                      })
                    }
            ...
            <box style={{ flexDirection: "column", marginTop: 1 }}>
              ...
              <input
                ...
                onChange={(value) =>
                  onUpdate({ repeatMode: normalizeRepeatMode(value) })
                }
                onKeyDown={(key) =>
                  handleRepeatCycleFromInputKey(key, "repeat_mode")
                }
            ...
            {recurrenceVisibility.showInterval ? (
              <box style={{ flexDirection: "column", marginTop: 1 }}>
                ...
                <input
                  ...
                  onKeyDown={(key) =>
                    handleRepeatCycleFromInputKey(key, "repeat_interval")
                  }
            ...
            {recurrenceVisibility.showWeekdays ? (
              <box style={{ flexDirection: "column", marginTop: 1 }}>
                ...
                <input
                  ...
                  onChange={(value) =>
                    onUpdate({ repeatWeekdays: parseWeekdaysInput(value) })
                  }
            ...
            {recurrenceVisibility.showCustomRule ? (
              <box style={{ flexDirection: "column", marginTop: 1 }}>
                ...
                <input
                  ...
                  onChange={(value) =>
                    onUpdate({ repeatCustomRRuleText: value })
                  }
            ...
            {recurrenceVisibility.showRepeatEnd ? (
              <box style={{ flexDirection: "column", marginTop: 1 }}>
                <text style={{ color: fieldLabelColor(true) }}>
                  REPEAT END (NEVER/UNTIL/COUNT)
                </text>
                <input
                  value={draft.repeatEndMode}
                  onChange={(value) =>
                    onUpdate({ repeatEndMode: normalizeEndMode(value) })
                  }
                  onKeyDown={(key) =>
                    handleRepeatCycleFromInputKey(key, "repeat_end_mode")
                  }
            ...
            <box style={{ flexDirection: "column", marginTop: 1 }}>
              <text
                style={{
                  color: checklistFocused ? theme.accentBlue : theme.muted,
                }}
              >
              ...
              {checklistItems.length === 0 ? (
                <text
                  style={{ color: checklistFocused ? theme.text : theme.muted }}
                >
            ...
            <box style={{ flexDirection: "column", marginTop: 1 }}>
              ...
              >
                ...
                >
                  {Array.from({ length: NOTES_VISIBLE_ROWS }).map(
                    (_, rowIndex) => {
                      const inThumb =
                        notesScrollbar.hasOverflow &&
                        rowIndex >= notesScrollbar.thumbTop &&
                        rowIndex <
                          notesScrollbar.thumbTop + notesScrollbar.thumbSize;
                      return (
                        <text
                          key={`notes-scroll-${rowIndex}`}
            ...
            <box style={{ flexDirection: "column", marginTop: 1 }}>
              ...
              <text style={{ color: theme.muted }}>
                Ctrl+N quick-captures from editor and auto-targets this task
                context.
              </text>
              <text style={{ color: theme.muted }}>
                Bridge inline NOTES into TOME using --from-task-notes (optional
                --clear-task-notes).
              </text>
            </box>
          </box>
        ...
        <box style={{ flexDirection: "row", gap: 1 }}>
          ...
          <box
            style={{
              ...
              paddingRight: 2,
              backgroundColor:
                focus === "cancel" ? theme.warn : theme.accentOrange,
              color: theme.bg,
            }}
            onMouseDown={(event) => {
