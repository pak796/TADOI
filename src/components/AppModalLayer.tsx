    ...
    >
      {uiState.modal.type === "delete" ? (
        uiState.modal.target === "regular_task" ? (
          ...
          >
            <text style={{ fontWeight: "bold" }}>
              DELETE RECURRING OCCURRENCE? [Y/F/N/ESC]
            </text>
            <text>Task: {uiState.modal.taskTitle}</text>
        ...
        >
          <text style={{ fontWeight: "bold" }}>
            DELETE THIS + FUTURE OCCURRENCES? [Y/N/ESC]
          </text>
          <text>Task: {uiState.modal.deleteModal.taskTitle}</text>
          <text>
            Occurrence: {uiState.modal.deleteModal.occurrenceIso.slice(0, 16)}
          </text>
          <ModalActionRow marginTop={1}>
            <ModalActionButton
              theme={modalTheme}
        ...
        <ModalContainer theme={theme} width={MODAL_STANDARD_WIDTH}>
          ...
          <text style={{ color: theme.muted }}>
            Task: {uiState.modal.taskTitle}
          </text>
          <input
            value={uiState.modal.value}
            onChange={(value) =>
              patchChecklistInputModal({ value, error: undefined })
            }
            onInput={(value) =>
              patchChecklistInputModal({ value, error: undefined })
            }
        ...
        <ModalContainer theme={theme} minWidth={MODAL_STANDARD_WIDTH}>
          <text style={{ color: theme.text, fontWeight: "bold" }}>
            CAPTURE TARGET HAS PRIMARY NOTE
          </text>
          <text style={{ color: theme.muted }}>
            Task: {uiState.modal.taskTitle}
          </text>
          <text style={{ color: theme.muted }}>
            Primary note: {uiState.modal.primaryNotePath}
          </text>
          <text style={{ color: theme.muted }}>
        ...
        <ModalContainer theme={theme} minWidth={MODAL_STANDARD_WIDTH}>
          ...
          <text style={{ color: theme.muted }}>
            {describeUnsavedSource(uiState.modal.source)}
          </text>
          <text style={{ color: theme.muted }}>
        ...
        <ModalContainer theme={theme} width={MODAL_STANDARD_WIDTH}>
          ...
          <text style={{ color: theme.muted }}>
            Tab: next field. Up/Down: move focus. Left/Right: cycle type.
        ...
        >
          ...
          <text>
            {formatLinkSnippet(uiState.modal.label, uiState.modal.target)}
          </text>
          <ModalActionRow marginTop={1}>
            <ModalActionButton
              theme={modalTheme}
        ...
        >
          ...
          >{`OPEN EXTERNAL SCHEME "${uiState.modal.scheme}"? [Y/N/ESC]`}</text>
