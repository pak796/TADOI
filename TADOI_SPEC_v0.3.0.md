...
- Runtime addendum (post-v0.3.0 docs sync): notifications now surface as actionable overdue popups (modal queue) and theme registry includes expanded accessibility/brand palettes.
...
3. **Filters**: status + due bucket + tag filtering (legacy single-cycle + boolean ALL/ANY/NONE).
...
58. **v0.3.0 version surfaces**: app version indicators and package metadata are aligned to `v0.3.0` / `0.3.0`.
...
- Nested boolean expressions beyond flat ALL/ANY/NONE buckets
...
- Maintain readability in 104×24.
...
type Filters = {
  ...
  tag?: string;        // legacy single include tag
  tagFilter?: { all?: string[]; any?: string[]; none?: string[] }; // boolean tag filter
...
- Package metadata in `package.json` matches the same release (`0.3.0`).
...
- Minimum supported size remains **104×24**.
- If below minimum, show a centered warning screen:
  - `Terminal too small (min 104x24) | Current: <WxH>`
...
1. **Below-min-size behavior**: shrink terminal below 104×24; verify clean “too small” message, no crash.
...
- Manual quality gate `T7.1` executed and passed:
  - Verified normal layout and interaction at `104x24`.
  - Verified centered guard at `103x23` with message:
    - `Terminal too small (min 104x24). Current: 103x23.`
...
Readability:
- Must remain legible at minimum supported `104x24`.
...
# Appendix J — v0.3.0 Interaction Polish (Mouse + Flash + Input Guard)
