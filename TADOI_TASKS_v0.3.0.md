...
- Current app version surfaces are aligned to `v0.3.0` / `0.3.0`.
...
## Phase R — Recurring Tasks (Schema v4)

### TR.1 Recurrence model + migration + validation
**Status**: Complete
**Implement**
- Extend `Task` with `recurrence` and `instance_of`.
- Add schema migration `v3 -> v4` with recurrence normalization and `series_id` backfill.
- Add strict persistence validation for recurrence/instance payloads.

**DoD**
- Legacy payloads migrate to schema v4.
- Invalid recurrence shapes are rejected in strict validation.
- Existing non-recurring tasks remain valid.

### TR.2 Recurrence engine + adapter
**Status**: Complete
**Implement**
- Add RRULE adapter module and recurrence engine helpers:
  - occurrence expansion in date ranges
  - next occurrence lookup
  - overdue occurrence lookup
  - EXDATE application

**DoD**
- Unit tests cover weekly BYDAY, monthly day-31 edge behavior, EXDATE exclusion, DST stability, and completion continuity.

### TR.3 Recurrence-aware visible row selector
**Status**: Complete
**Implement**
- Add `buildVisibleTaskRows(...)` selector that merges:
  - regular tasks
  - virtual recurring occurrences
  - materialized instance rows
- Enforce due-window expansion policy and one-actionable-occurrence policy for `due=any` and `due=overdue`.

**DoD**
- List/dashboard consume the selector as single source of truth.
- Matching materialized instances suppress corresponding virtual rows.

### TR.4 Occurrence actions + key routing
**Status**: Complete
**Implement**
- Add keys:
  - `x`: skip selected occurrence
  - `z`: snooze selected occurrence
  - `E`: edit series
- Keep `e` as occurrence edit on recurring rows.
- Keep completion semantics occurrence-scoped for recurring rows.

**DoD**
- Key router tests confirm routing and no leakage.
- Recurring occurrence actions mutate state as expected without closing series.

### TR.5 Editor + details/list/dashboard integration
**Status**: Complete
**Implement**
- Editor recurrence controls and preview.
- Recurrence indicator in task rows.
- Details pane recurrence summary and occurrence context.
- Dashboard/KPI/top-tags consume recurrence-aware rows.

**DoD**
- Recurring behavior visible and actionable across list/details/dashboard.
- Existing non-recurring flows remain unchanged.
...
### T7.1 104x24 & resize smoke test
**Status**: Complete
**DoD**
- UI usable at 104x24.
- Resize does not crash or overlap catastrophically.
**QA Notes (2026-02-09 CST)**
- Verified baseline render at `104x24` using `stty size` and `bun run dev`; full rail/list/details layout rendered and remained interactive.
- Verified `<104x24` guard at `103x23`; app rendered centered warning: `Terminal too small (min 104x24). Current: 103x23.` with `Resize terminal to continue` and `Press q to quit`.
- After returning terminal to `104x24`, normal UI resumed on next launch with no crash and no broken persisted state.
...
## Nice-to-haves (v1.1)
- Nested boolean tag expressions beyond flat ALL/ANY/NONE buckets
...
## T12.3 Version surfaces sync to v0.3.0
**Status**: Complete
**Implement**
- Update `APP_VERSION` to `v0.3.0`.
- Update `package.json` version to `0.3.0`.
- Keep left rail and help pane bound to centralized `APP_VERSION`.

**DoD**
- Left rail shows `v0.3.0`.
- Help pane displays `App Version: v0.3.0`.
...
- Re-affirm minimum terminal size: 104×24.
- Ensure below-min-size behavior is documented (message, no overlap).

**DoD**
- Docs clearly state support matrix and 104×24 minimum.
...
- Add a guard in layout/render pipeline:
  - If terminal < 104×24: render a single centered warning screen.
  - Disable other interactions to avoid crashes/layout churn.
- Ensure resizing back above min restores the full UI.

**DoD**
- Shrinking below 104×24 shows a stable “Terminal too small” message.
...
- `App` now renders a centered guard screen below `104x24` and blocks normal key routing while too small.
...
# Phase 17 — v0.3.0 Interaction Polish (Mouse + Input Guard)
...
# Phase 18 — Post-v0.3.0 Dashboard + UX Alignment
...
> Scope note: this phase captures post-v0.3.0 runtime updates for overdue notification UX and expanded theme catalog coverage.
