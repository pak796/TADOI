...
- Import foundation is implemented in service modules (`icsParser`, `importMapper`, `calendarImportService`) with dry-run/report/mode support.
- `calendar:import` is not yet exposed as a top-level CLI route (import is currently surfaced through Backup Center).
